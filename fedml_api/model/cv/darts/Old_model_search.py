...
class MobileNet_Network(nn.Module):
    ...
    def __init__(self, backbone_model, batch_size, C, num_classes, layers, criterion, device, steps=4, multiplier=4,
        ...
        C_curr = stem_multiplier * C  # 3*16
        self.stem = nn.Sequential(
            nn.Conv2d(2560, C_curr, 3, padding=1, bias=False),
            nn.BatchNorm2d(C_curr)
        )
    ...
    def forward(self, input):
        ...
        mixed_hidden_feature = self.backbone_model(input)
        logging.info("Size of mixed_hidden_feature " + str(mixed_hidden_feature.size()))
        s0 = s1 = self.stem(mixed_hidden_feature)
        #logging.info("Self Cell " + str(self.cells.__sizeof__()))
        for i, cell in enumerate(self.cells):
            logging.info("i count " + str(i))
            ...
            if cell.reduction:
                ...
                logging.info("weights " + str(weights.size()))
            s0, s1 = s1, cell(s0, s1, weights)
        out = self.global_pooling(s1)
        logits = self.classifier(out.view(out.size(0), -1))
        logging.info("returning logit")
        return logits
