...
class MixedOp(nn.Module):
    ...
    def forward(self, x, weights):
        # logging.info("Mixed Operation")
        ## Original
        # w is the operation mixing weights. see equation 2 in the original paper.
        # logging.info(sum(w * op(x) for w, op in zip(weights, self._ops)))
        ...
        # if len(clist) == 1:
        #     logging.info(clist[0])
        #     return clist[0]
        # else:
        #     logging.info(sum(clist))
        #     return sum(clist)
...
class FedNASNetwork(nn.Module):
    ...
    def base_arch_weight_parameters(self):
        return self.backbone_model.load_state()
