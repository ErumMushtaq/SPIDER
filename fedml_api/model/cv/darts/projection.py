...
import numpy as np
import torch
import fedml_api.model.cv.darts.utils_2 as ig_utils
import logging
import torch.utils
...
def project_op(model, proj_queue, args, device, infer, cell_type, selected_eid=None):
    ''' operation '''
    #### macros
    num_edges, num_ops = model.num_edges, model.num_ops
    candidate_flags = model.candidate_flags[cell_type]
    proj_crit = args.proj_crit[cell_type] # projection criteria accuracy

    #logging.info('number of edges '+str(num_edges)+' number of operations '+str(num_ops)+' candidate flags '+str(candidate_flags)+' project_crit '+str(proj_crit))
    ...
    for opid in range(num_ops):
        ...
        valid_stats = infer(proj_queue, model, device, args, log=False, _eval=False, weights_dict=weights_dict)
        ...
        # logging.info(' operation %f', opid)
        # logging.info(' Edge %f', selected_eid)
...
def project_edge(model, proj_queue, args, device, infer, cell_type):
    ''' topology '''
    #### macros
    # logging.info(model.candidate_flags)
    # logging.info(model.get_projected_weights(cell_type))
    candidate_flags = model.candidate_flags_edge[cell_type]
    proj_crit = args.proj_crit[cell_type]
    # logging.info('project edge')
    # logging.info(candidate_flags)

    #### select an edge
    remain_nids = torch.nonzero(candidate_flags).cpu().numpy().T[0]
    # logging.info(remain_nids)
    if args.edge_decision == "random":
...
def pt_project(train_queue, valid_queue, model, architect, device, optimizer,
               epoch, args, infer, perturb_alpha, epsilon_alpha):
    model.train()
    model.printing(logging)

    train_acc, train_obj = infer(train_queue, model, device, args, log=False)
    # logging.info('train_acc  %f', train_acc)
    # logging.info('train_loss %f', train_obj)

    valid_acc, valid_obj = infer(valid_queue, model, device, args, log=False)
    ...
    #### reset optimizer
    model.reset_optimizer(args.lr / 10, args.momentum, args.weight_decay)
    ...
    for epoch in range(start_epoch, tune_epochs):
        ...
        ## project
        # proj_intv interval between two projections
        if epoch % proj_intv == 0 or epoch == tune_epochs - 1:
            ...
            if epoch < proj_intv * model.num_edges:
                ...
                selected_eid_normal, best_opid_normal = project_op(model, proj_queue, args, device, infer, cell_type='normal')
                model.project_op(selected_eid_normal, best_opid_normal, cell_type='normal')
                selected_eid_reduce, best_opid_reduce = project_op(model, proj_queue, args, device, infer, cell_type='reduce')
                model.project_op(selected_eid_reduce, best_opid_reduce, cell_type='reduce')

                model.printing(logging)
            else:
                # logging.info('project edge')
                logging.info(model.get_projected_weights(cell_type='normal'))
                selected_nid_normal, eids_normal = project_edge(model, proj_queue, args, device, infer, cell_type='normal')
                # logging.info(eids_normal)
                # logging.info(selected_nid_normal)
                model.project_edge(selected_nid_normal, eids_normal, cell_type='normal')
                # logging.info(model.get_projected_weights(cell_type='normal'))
                selected_nid_reduce, eids_reduce = project_edge(model, proj_queue, args, device, infer, cell_type='reduce')
