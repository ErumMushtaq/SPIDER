...
class FedNAS_Network(nn.Module):
    ...
    def new(self):
        model_new = FedNAS_Network(self._C, self._num_classes, self._layers, self._criterion).to(
            self.device)
    ...
    def forward(self, input):
        ...
        for i, cell in enumerate(self.cells):
            ...
            # logging.info("s0.shape = %s" % str(s0.shape))
            # logging.info("s1.shape = %s" % str(s1.shape))
