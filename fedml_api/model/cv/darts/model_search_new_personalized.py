...
class Backbone_Cell_Network_(nn.Module):

    def __init__(self, backbone_model, batch_size, C, num_classes, layers, criterion, device, args, steps=4, multiplier=4, stem_multiplier=3):
        super(Backbone_Cell_Network_, self).__init__()
        # print(Network)
    ...
    def new(self):
        model_new = Backbone_Cell_Network_(self._C, self._num_classes, self._layers, self._criterion, self.device).to(self.device)
