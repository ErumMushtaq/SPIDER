...
def pt_project(train_queue, valid_queue, model, architect, device, optimizer,
    ...
    # (train_queue, valid_queue, model, architect, device, optimizer,
    #  epoch, args, infer, perturb_alpha, epsilon_alpha)
    def project(model, args):
        ...
        for opid in range(num_op):
            ...
            ## proj evaluation
            valid_stats = infer(valid_queue, model, device, log=False, eval=False, weights=weights)
    ...
    train_acc, train_obj = infer(train_queue, model, device, log=False)
    logging.info('train_acc  %f', train_acc)
    logging.info('train_loss %f', train_obj)

    valid_acc, valid_obj = infer(valid_queue, model, device, log=False)
