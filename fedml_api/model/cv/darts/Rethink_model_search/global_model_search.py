...
# from fedml_api.model.cv.darts.bn_operation import *
...
class Global_Network(nn.Module):
    def __init__(self, C, num_classes, layers, criterion, primitives, args, device,
        ...
        #### optimizer
        self._args = args
        self.k = sum(1 for i in range(self._steps) for n in range(2 + i)) #number of edges
        self.num_ops = len(self.PRIMITIVES['primitives_normal'][0])    # number of operations
            # self.num_edges = k
            # self.num_ops = num_ops
    ...
    def _initialize_alphas_numpy(self, k, num_ops):
        ...
        return Variable(1e-3 * torch.ones(self.k, self.num_ops).to(self.device), requires_grad=True)
    ...
    # #     # if float(1.0) in weights_normal:
    # #     number = torch.ones(1)
    # #     non_zero_indices = weights_normal == number
    # #     weights_normal[non_zero_indices] = 0.5

    # #     non_zero_indices = weights_reduce == 1.0
    # #     weights_reduce[non_zero_indices] = 0.5

    # #     logging.info(non_zero_indices)

    #     return {'normal': weights_normal, 'reduce': weights_reduce}

    def get_softmax(self):
        weights_normal = torch.ones(self.k, self.num_ops)
        weights_reduce = torch.ones(self.k, self.num_ops)
        return {'normal': weights_normal, 'reduce': weights_reduce}
    ...
    def new(self):
        model_new = Global_Network(self._C, self._num_classes, self._layers, self._criterion, self.PRIMITIVES, self._args, \
