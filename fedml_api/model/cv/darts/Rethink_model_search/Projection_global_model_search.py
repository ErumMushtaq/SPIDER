...
class GDartsNetworkProj(Global_Network):
    def __init__(self, C, num_classes, layers, criterion, primitives, args, device,
        ...
        super(GDartsNetworkProj, self).__init__(C, num_classes, layers, criterion, primitives, args, device,
        ...
        logging.info('Model Initializzzed')
    ...
    def _initialize_flags(self):
        self.candidate_flags = {
            'normal': torch.tensor(self.num_edges * [True], requires_grad=False, dtype=torch.bool).to(self.device),
            'reduce': torch.tensor(self.num_edges * [True], requires_grad=False, dtype=torch.bool).to(self.device),
        }  # must be in this order
        self.candidate_flags_edge = {
            'normal': torch.tensor(3 * [True], requires_grad=False, dtype=torch.bool).to(self.device),
            'reduce': torch.tensor(3 * [True], requires_grad=False, dtype=torch.bool).to(self.device),
        }
    ...
    def project_op(self, eid, opid, cell_type):
        # self.proj_weights[cell_type][eid][opid] = 0.5  ## hard by default
    ...
    def return_proj_weights(self):
        return self.proj_weights, self.candidate_flags, self.nid2selected_eids, self.candidate_flags_edge

    def set_proj_weights(self, proj_weights, candidate_flags, nid2selected_eids, candidate_flags_edges):
        self.proj_weights = proj_weights
        self.candidate_flags = candidate_flags
        self.nid2selected_eids = nid2selected_eids
        self.candidate_flags_edge = candidate_flags_edges

    def forward(self, input, weights_dict=None, update_operations = 0.):
        if weights_dict is None or 'normal' not in weights_dict:
            weights_normal = self.get_projected_weights('normal')
        else:
            weights_normal = weights_dict['normal'] #used for inference
        ...
        for i, cell in enumerate(self.cells):
            ...
            if update_operations == 1:
                s0, s1 = s1, cell(s0, s1, weights, self.drop_path_prob, 1)
                #logging.info('operations updated')
            else:
