...
class MixedOp(nn.Module):
    ...
    def forward(self, x, weights, cpu_weights):
        ...
        for j, cpu_weight in enumerate(cpu_weights):
            if abs(cpu_weight) > 1e-10:
                clist.append(weights[j] * self.m_ops[j](x))
        # logging.info(" Mixed Operations")
        # logging.info(len(clist))
        if len(clist) == 1:
            return clist[0]
        else:
...
class Gumbel_Network(nn.Module):
    ...
    def forward(self, input):
        ...
        for i, cell in enumerate(self.cells):
            #logging.info(" Before Alphas ")
            if cell.reduction:
                #logging.info(self.alphas_reduce)
                if self.alphas_reduce.size(1) == 1:
                    #weights = F.softmax(self.alphas_reduce, dim=0)
                    weights = F.gumbel_softmax(self.alphas_reduce, self.tau,  dim=0)
                    #weights = F.gumbel_softmax(self.alphas_reduce, self.tau, hard = True, dim = 0)
                else:
                    #weights = F.softmax(self.alphas_reduce, dim=-1)
                    weights = F.gumbel_softmax(self.alphas_reduce, self.tau,  dim=-1)
                    #weights = F.gumbel_softmax(self.alphas_reduce, self.tau, hard=True, dim=-1)
            else:
                #logging.info(self.alphas_normal)
                if self.alphas_normal.size(1) == 1:
                    #weights = F.softmax(self.alphas_normal, dim=0)
                    weights = F.gumbel_softmax(self.alphas_normal, self.tau, dim=0)
                    #weights = F.gumbel_softmax(self.alphas_normal, self.tau, hard=True, dim=0)
                else:
                    #weights = F.softmax(self.alphas_normal, dim=-1)
                    weights = F.gumbel_softmax(self.alphas_normal, self.tau,  dim=-1)
                    #weights = F.gumbel_softmax(self.alphas_normal, self.tau, hard=True, dim=-1)
            # logging.info(" Gumbel Softmax ")
            # logging.info(weights)
