...
class Network_Train(nn.Module):
    ...
    def new(self):
        model_new = Network_Train(self._C, self._num_classes, self._layers, self._criterion, self.device).to(self.device)
    ...
    def forward(self, input):
        ...
        for i, cell in enumerate(self.cells):
            if cell.reduction:
                #weights = self.normal_mask
                # weights = torch.ones(self.k, self.num_ops)/self.num_ops
                weights = F.softmax(self.alphas_reduce)
                #logging.info(weights.size())
            else:
                #weights = self.reduction_mask
                # weights = torch.ones(self.k, self.num_ops)/self.num_ops
                #weights = torch.ones(8, 1)
                weights = F.softmax(self.alphas_normal)
                #logging.info(weights.size())
    ...
    def set_original_mask(self):
        k = sum(1 for i in range(self._steps) for n in range(2 + i))
        num_ops = len(PRIMITIVES)
        self.normal_mask = torch.ones(k, num_ops)
        self.reduction_mask = torch.ones(k, num_ops)

    def update_mask(self, normal_mask, reduction_mask):
