...
class Network_FedNAS(nn.Module):

    # def __init__(self, C, num_classes, layers, criterion, device, steps=4, multiplier=4, stem_multiplier=3):
    def __init__(self, C, num_classes, layers, criterion, device, args, steps=4,
                 multiplier=4,
                 stem_multiplier=3):
        super(Network_FedNAS, self).__init__()
        self._C = C
    ...
    def new(self):
        model_new = Network_FedNAS(self._C, self._num_classes, self._layers, self._criterion, self.device, self.args).to(
            self.device)
    ...
    def forward(self, input):
        #logging.info("FedNAS workshop model called but with Gumbel softmax")
