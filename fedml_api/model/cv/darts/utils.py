...
def count_parameters_in_Compact(model):
    from fedml_api.model.cv.darts.final_model_rethink import Network as CompactModel
...
def drop_path(x, drop_prob):
    if drop_prob > 0.:
        ...
        # logging.info(device)
        torch_var = torch.FloatTensor(x.size(0), 1, 1, 1).to(device)
        mask = Variable(torch_var.bernoulli_(keep_prob))
...
class KL_Loss(nn.Module):
    ...
    def forward(self, output_batch, teacher_outputs):
        ...
        # print('loss H:',loss_2)
        # logging.info("size of student logit "+str(output_batch.shape))
        # logging.info("size of student logit " + str(teacher_outputs.shape))
