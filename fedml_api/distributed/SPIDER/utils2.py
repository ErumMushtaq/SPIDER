...
def save_training_model(args, personalized_model, client_index):
    # create folder
    folder = args.personalized_model_path + "_" + str(args.partition_method) + "_" + str(args.partition_alpha) + "_" + str(args.dataset)+"_"+ str(args.run_id)+ "/Training_model"
    if not os.path.exists(folder):
        os.mkdir(folder)
    path = folder + '/client_' + str(client_index) + '.pth'
    torch.save(personalized_model.cpu().state_dict(), path)  # save the model
...
def save_extra_variables(args, extra_variables, client_index):
    # create folder
    folder = args.personalized_model_path + "_" + str(args.partition_method) + "_" + str(args.partition_alpha) + "_" + str(args.dataset) +"_"+ str(args.run_id)+ "/Extra_Variables"
    if not os.path.exists(folder):
        os.mkdir(folder)
...
def load_extra_variables(args, client_index):
    path = args.personalized_model_path + "_" + str(args.partition_method) + "_" + str(args.partition_alpha) + "_" + str(args.dataset) +"_"+ str(args.run_id)+ '/Extra_Variables/client_' + str(client_index) + '.pkl'
...
def load_training_model(args, personalized_model, client_index):
    path = args.personalized_model_path + "_" + str(args.partition_method) + "_" + str(args.partition_alpha) + "_" + str(args.dataset)+"_"+ str(args.run_id) + '/Training_model/client_' + str(client_index) + '.pth'
...
def load_personal_model(args, personalized_model, client_index):
    path = args.personalized_model_path + "_" + str(args.partition_method) + "_" + str(args.partition_alpha) + "_" + str(args.dataset) +"_"+ str(args.run_id)+ '/personalized_model/client_' + str(client_index) + '.pth'
    if os.path.exists(path):  # checking if there is a file with this name
        personalized_model.load_state_dict(torch.load(path))  # if yes load it
        # logging.info(" Personal Model of Client number %d Loaded " % client_index)
...
def load_personal_train_model(args, personalized_model, client_index):
    path = args.personalized_model_path + "_" + str(args.partition_method) + "_" + str(args.partition_alpha) + "_" + str(args.dataset) +"_"+ str(args.run_id)+'/Train_folder/personalized_model/client_' + str(client_index) + '.pth'
    if os.path.exists(path):  # checking if there is a file with this name
        personalized_model.load_state_dict(torch.load(path))  # if yes load it
        logging.info(" Personal Model of Client number %d Loaded " % client_index)
